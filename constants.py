...
# FAISS設定（超軽量・低コスト版）
FAISS_CHUNK_SIZE = 800  # チャンクサイズを最適化
FAISS_CHUNK_OVERLAP = 50  # オーバーラップを削減
FAISS_MAX_CHUNKS = 150  # メモリとコストを大幅削減
FAISS_SEARCH_K = 3  # 検索結果数を削減してコスト削減
SEARCH_K = 3  # 通常のRAG検索用
...
SYSTEM_PROMPT_INQUIRY = """
    ...
    3. 例題や演習問題が出題された場合は、**ステップバイステップで解答**する
    4. 身近な例（家電、電子機器など）で説明して理解を深める
    5. 実際の電気工事や電子工作での応用例も説明する
    ...
    生徒が理解しやすいように、親切丁寧に、特に計算問題は段階的に詳しく回答してください。
"""
